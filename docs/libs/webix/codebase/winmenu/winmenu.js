webix.protoUI({
	...
	type:{
		template:function(obj){
			// var result = "";
			// if (obj.icon){
			// 	result = "<span class='webix_icon " + obj.icon + "'></span><div class='title'>" + obj.value + "</div>";
			// }
			// else if (obj.img){
			...
			if(obj.img_full){
				...
				image = "<span class='menu_icon "+obj.icon+"'></span>";
			}
			else if(obj.img){
				image = "<img src='"+obj.img+"' align='center'>";
			}
			if(obj.badge && obj.badge>0){
				image += "<span class='webix_badge'>"+obj.badge+"</span>";
			}
